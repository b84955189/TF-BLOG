...
小型信息管理系统。
## 演示地址(已失效)
[http://blog.lking.icu](https://blog.lking.icu)

## 必要配置
1. 重命名` src/main/resources `目录下的`example_druid.properties`文件为`druid.properties`
打开填写数据库信息即可。
```
#可忽略druid可自动识别
#driverClassName=com.mysql.jdbc.Driver
url=jdbc:mysql://地址:端口/数据库?useSSL=false&serverTimezone=UTC&useUnicode=true&characterEncoding=utf8
# 数据库账号
username=root
#数据库密码
password=xxx
...
maxWait=3000
```
2. 重命名` src/main/resources `目录下的`example_qiniu.properties`文件为`qiniu.properties`
打开填写七牛密钥信息即可。
```
access_key=xxx
secret_key=xxx
...
```
3. 导入验证码外部jar包
由于验证码为个人编写，另外也没有导入Maven仓库，因此只能手动导入。
因此注意导入在本项目目录`ExternalFile`下的验证码jar包。


4. SQL文件

> 管理后台：http://{url}/admin 默认admin 密码：123
```sql
